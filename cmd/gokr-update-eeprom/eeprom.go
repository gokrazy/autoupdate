...
var (
	...
	remoteFirmwareDir = flag.String("remote_firmware_dir",
...
func main() {
	...
	eepromRef, err := os.ReadFile("_build/eeprom-ref.txt")
	if err != nil {
		log.Fatal(err)
	}
	...
	contents, err := githubContents("https://api.github.com/repos/raspberrypi/rpi-eeprom/contents/" + *remoteFirmwareDir + "?ref=" + strings.TrimSpace(string(eepromRef)))
	if err != nil {
		log.Fatal(err)
	}
